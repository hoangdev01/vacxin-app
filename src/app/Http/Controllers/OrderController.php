...
use App\Mail\ConfirmPaidOrder;


class OrderController extends Controller
{
	public function getOrder() {
		$orders = Order::paginate(config('const.per_page'));
	...
	public function updateState(Request $request, $id) {
		...
		if($state->state) Mail::to($state->customer_email)->send(new ConfirmPaidOrder($state->id, $state->total));
		$state->save();
		return response()->json([
			'message' =>false
		]);
	...
	public function update(UpdateOrderRequest $request, $id)
	{
		...
		$price = Vaccine::find($request->vaccine)->late_price;
		$data['total'] = $price*($request->quantity);
		$data['state'] = Vaccine::STATUS_INACTIVE;
